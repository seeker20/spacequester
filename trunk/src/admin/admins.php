...
if(isset($_GET['action'])||isset($_POST['action'])) {
	if($_GET['action'] == "delete") {
		$res = $db->query('SELECT name FROM '.SYSTEM_dbpref."admin WHERE id = '".AntiHacker($_GET['id'])."';");
		$dbsatz = $db->get($res);
		$admin = $dbsatz['name'];
		$db->query("DELETE FROM ".SYSTEM_dbpref."admin WHERE id = '".AntiHacker($_GET['id'])."';");
			...
			if($_POST['username'] != "") {
				if($_POST['password'] != "") {
					...
					if($db->count($db->query("SELECT * FROM ".SYSTEM_dbpref."admin WHERE name='$username'",true))>0) {
						...
						$password = AntiHacker(md5($password));
		...
		if(isset($_POST['username']) && isset($_POST['password'])) {
			if($_POST['username'] != "") {
				$username = AntiHacker($_POST['username']);
				$password = AntiHacker($_POST['password']);
					
				$dsatz    = $db->get($db->query("SELECT * FROM ".SYSTEM_dbpref."admin WHERE id='" . AntiHacker($_POST["id"]) . "'",true));
				...
				if($db->count(($res=$db->query("SELECT * FROM ".SYSTEM_dbpref."admin WHERE name='$username'",true)))>0) {
					...
					if($dsatz['id']!=$_POST['id']) {
						...
						$res = $db->query("UPDATE ".SYSTEM_dbpref."admin SET name='$username', password='$password' WHERE id='" . AntiHacker($_POST['id']) . "'");
...
if(isset($_GET["id"])||isset($_POST["id"])) {
	...
	$db->query("SELECT * FROM ".SYSTEM_dbpref."admin WHERE id = '".AntiHacker($_GET['id'])."';");
	$dsatz = $db->get();
...
while($dsatz=$db->get($res)) {
	$tpl->TextRepeater('list', array('admin_name','admin_ip','ADMINID'),array(ReAntiHacker($dsatz['name']),ReAntiHacker($dsatz['ip']),ReAntiHacker($dsatz["id"])));
}
$tpl->clearList('list');
$tpl->out();
?>