...
if(isset($_GET["ne"])) {
	...
	echo "<td><input type='text' name='log' value='0'></td>";
	echo "</tr>\n";
	echo "<tr>";
	...
	if($name==""||$name=="Linkname") {
		...
		$db->query("INSERT INTO `" . SYSTEM_dbpref . "menue` ( `id` , `name` , `target` , `log` ) VALUES ( NULL , '$name', '$nstarget', '$log' )");
...
if(isset($_GET["ed"])) {
	...
	echo "<td><input type='text' name='log' value='".$dsatz["log"]."'></td>";
	echo "</tr>\n";
	echo "<tr>";
	...
	if($name==""||$name=="Linkname") {
		...
		$db->query("UPDATE `" . SYSTEM_dbpref . "menue` SET `name` = '$name', `target`= '$nstarget', `log`= '$log' WHERE `id` = '$id'");
...
<tr align='center'><th style='border-width:1px;border-style:solid;'>Eintrag Name</th><th style='border-width:1px;border-style:solid;'>Ziel</th><th style='border-width:1px;border-style:solid;'>Ort</th><th style='border-width:1px;border-style:solid;'>Aktion</th></tr>
<?php
//Ausgabe
...
while($dsatz=$db->get($res)) {
	...
	echo "<td style='border-width:1px;border-style:solid;'>".$dsatz["log"]."</td>";
